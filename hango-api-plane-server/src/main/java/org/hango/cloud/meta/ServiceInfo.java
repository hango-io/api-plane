...
public class ServiceInfo {
    ...
    // 也可直接从API中获得apiName
    private String apiName = wrap(TemplateConst.SERVICE_INFO_API_NAME);

    private String serviceName = wrap(TemplateConst.SERVICE_INFO_API_SERVICE);

    private String gateway = wrap(TemplateConst.SERVICE_INFO_API_GATEWAY);

    // 提供method占位符，例如${t_api_methods},后续GatewayModelProcessor会进行渲染
    // 也可直接从API中获得method
    private String method = wrap(TemplateConst.SERVICE_INFO_API_METHODS);

    // 提供uri占位符，例如${t_api_request_uris},后续GatewayModelProcessor会进行渲染
    // 也可直接从API中获得uri
    private String uri = wrap(TemplateConst.SERVICE_INFO_API_REQUEST_URIS);

    // 提供subset占位符，例如${t_virtual_service_subset_name},后续GatewayModelProcessor会进行渲染，不能从API中获得
    private String subset = wrap(TemplateConst.SERVICE_INFO_VIRTUAL_SERVICE_SUBSET_NAME);

    private String hosts = wrap(TemplateConst.SERVICE_INFO_VIRTUAL_SERVICE_HOST_HEADERS);

    private String priority = wrap(TemplateConst.SERVICE_INFO_VIRTUAL_SERVICE_PLUGIN_MATCH_PRIORITY);
