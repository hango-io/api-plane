    ...
    public FragmentHolder process(String plugin, ServiceInfo serviceInfo) {
        ...
        if (source.contain("$.condition.request.headers")) {
            ...
            headers.forEach(item -> {
                ...
                if ("safe_regex_match".equals(matchType)) {
                    builder.addJsonElement("$.downgrade_rqx.headers", String.format("{\"name\":\"%s\","
        ...
        if (source.contain("$.condition.request.host")) {
            ...
            if (nonNull(matchType, host)) {
                if ("safe_regex_match".equals(matchType)) {
                    builder.addJsonElement("$.downgrade_rqx.headers", String.format("{\"name\":\":authority\","
                                                                                    + "\"safe_regex_match\":{\"regex"
                                                                                    + "\":\"%s\"}}", host));
                } else {
        ...
        if (source.contain("$.condition.request.method")) {
            ...
            if (nonNull(method)) {
                if (method.size() == 1) {
                    ...
                    builder.addJsonElement("$.downgrade_rqx.headers", String.format("{\"name\":\":method\","
                                                                                    + "\"safe_regex_match\":{\"regex"
                                                                                    + "\":\"%s\"}}", String.join("|", method)));
                }
            }
        }
        if (source.contain("$.condition.request.path")) {
            ...
            if (nonNull(matchType, path)) {
                if ("safe_regex_match".equals(matchType)) {
                    builder.addJsonElement("$.downgrade_rqx.headers", String.format("{\"name\":\":path\","
                                                                                    + "\"safe_regex_match\":{\"regex"
                                                                                    + "\":\"%s\"}}", path));
                } else {
        ...
        if (source.contain("$.condition.response.code")) {
            ...
            if (nonNull(code)) {
//                builder.addJsonElement("$.downgrade_rpx.headers", String.format("{\"name\":\":status\",\"safe_regex_match\":\"%s|\"}", code));
                builder.addJsonElement("$.downgrade_rpx.headers", String.format("{\"name\":\":status\","
                                                                                + "\"safe_regex_match\":{\"regex"
                                                                                + "\":\"%s|\"}}", code));
            }
        }
