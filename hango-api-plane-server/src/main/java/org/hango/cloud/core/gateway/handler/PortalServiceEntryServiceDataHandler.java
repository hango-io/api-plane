...
public class PortalServiceEntryServiceDataHandler extends ServiceDataHandler {
    ...
    List<TemplateParams> doHandle(TemplateParams tp, Service service) {
        ...
        if (Const.PROXY_SERVICE_TYPE_STATIC.equals(service.getType())) {
            ...
            params.put(TemplateConst.SERVICE_ENTRY_PROTOCOL, protocol);
            params.put(TemplateConst.SERVICE_ENTRY_PROTOCOL_NAME, protocolName);
            params.put(TemplateConst.SERVICE_ENTRY_PROTOCOL_PORT, protocolPort);
            params.put(TemplateConst.GATEWAY_GW_CLUSTER, service.getGateway());

            params.put("endpoints", endpoints)
                    .put(TemplateConst.SERVICE_ENTRY_NAME, service.getCode().toLowerCase())
                    .put(TemplateConst.SERVICE_ENTRY_HOST, host);
        }
